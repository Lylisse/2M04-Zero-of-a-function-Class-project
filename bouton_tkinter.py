...
#on choisit quelle méthode utilisée puis envoyons en arguments la fonction et l'intervalle sous la forme suivante:"[{fonction}, {début de l'intervalle},  {fin de l'intervalle}]"
    ...
    if method.get() == "method1":
        pass
    elif method.get() == "method2":
        pass
    else:
...
ttk.Checkbutton(mainframe, text="méthode 1", variable=methode_1_select, onvalue="method1").grid(column=1, row=3)


ttk.Checkbutton(mainframe, text="méthode 2", variable=methode_1_select, onvalue="method2").grid(column=2, row=3)


ttk.Button(mainframe, text="calculate_zeros", command=lambda:calculate_zeros(methode_1_select, fonction_entry, [debut_intervalle_entry.get(), fin_intervalle_entry.get()])).grid(column=1, row=4, sticky=W)
#le bouton permettant de lancer le calcul des zéros sera placé à un certain endroit

errorLabel = ttk.Label(mainframe, text="no error detected")
errorLabel.grid(row=4, column=3)
